...
(function (astrology) {
	...
	// Per-planet colors (matching PlanetWatcher style)
	astrology.PLANET_COLORS = {
		Sun: "#e8a500",
		Moon: "#808080",
		Mercury: "#c4b200",
		Venus: "#228B22",
	...
	astrology.INDOOR_CIRCLE_RADIUS_RATIO = 1.5;
	...
	// Aspect line stroke width
	astrology.ASPECTS_STROKE = 1.5;
	...
	// Aspects
	astrology.ASPECTS = {
		conjunction: { degree: 0, orbit: 10, color: "#FF4500" },
		sextile: { degree: 60, orbit: 6, color: "#0000CC" },
	...
	astrology.RULER_DEGREES = 1;
	astrology.LARGE_RULER_EVERY = 10;
...
(function (astrology) {
	...
	astrology.SVG.prototype.getSymbol = function (name, x, y) {
		...
		// Apply per-planet color if defined
		if (symbol && astrology.PLANET_COLORS && astrology.PLANET_COLORS[name]) {
			var planetColor = astrology.PLANET_COLORS[name];
			var children = symbol.querySelectorAll("path, circle, line");
			for (var ci = 0; ci < children.length; ci++) {
				children[ci].setAttribute("stroke", planetColor);
			}
			return symbol;
		}
		if (symbol) {
	...
	function jupiter(x, y) {
		...
		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x, y - 3));
	...
	function aries(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[0]);
		...
		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x, y - 4));
	...
	function taurus(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[1]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function gemini(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[2]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function cancer(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[3]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);

		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x - 18, y));
		return wrapper;
	}
	...
	function leo(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[4]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);

		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x - 6, y - 13));
		return wrapper;
	}
	...
	function virgo(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[5]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function libra(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[6]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);

		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x - 6, y));
		return wrapper;
	}
	...
	function scorpio(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[7]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function sagittarius(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[8]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);

		if (astrology.ADD_CLICK_AREA)
			wrapper.appendChild(createRectForClick(x - 12, y));
		return wrapper;
	}
	...
	function capricorn(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[9]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function aquarius(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[10]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
	...
	function pisces(x, y) {
		...
		node.setAttribute("stroke", astrology.SIGNS_COLOR[11]);
		node.setAttribute("stroke-width", astrology.SIGNS_STROKE);
		node.setAttribute("fill", "none");
		wrapper.appendChild(node);
...
(function (astrology) {
	...
	astrology.Chart = function (elementId, width, height, settings) {
		if (settings) {
			...
			if (!("COLORS_SIGNS_FILL" in settings))
				astrology.COLORS_SIGNS_FILL = [
					astrology.COLOR_ARIES,
					astrology.COLOR_TAURUS,
					astrology.COLOR_GEMINI,
...
(function (astrology) {
	...
	astrology.Radix = function (paper, cx, cy, radius, data) {
		...
		var divisionForAspects = document.createElementNS(
			this.paper.root.namespaceURI,
			"g"
		);
		divisionForAspects.setAttribute(
			"id",
			astrology._paperElementId + "-" + astrology.ID_ASPECTS
		);
		...
		this.universe.setAttribute(
			"id",
			astrology._paperElementId + "-" + astrology.ID_RADIX
		);
	...
	astrology.Radix.prototype.drawBg = function () {
		...
		var wrapper = astrology.utils.getEmptyWrapper(
			universe,
			astrology._paperElementId + "-" + astrology.ID_BG
		);
		...
		hemisphere.setAttribute(
			"fill",
			astrology.STROKE_ONLY ? "none" : astrology.COLOR_BACKGROUND
		);
	...
	astrology.Radix.prototype.drawUniverse = function () {
		...
		for (
			var i = 0,
				step = 30,
				start = this.shift,
				len = astrology.COLORS_SIGNS_FILL.length;
			i < len;
			i++
		) {
			...
			segment.setAttribute(
				"fill",
				astrology.STROKE_ONLY ? "none" : astrology.COLORS_SIGNS_FILL[i]
			);
			segment.setAttribute(
				"id",
				astrology._paperElementId +
			...
			segment.setAttribute(
				"stroke",
				astrology.STROKE_ONLY ? astrology.CIRCLE_COLOR : "none"
			);
		...
		for (
			var i = 0,
				step = 30,
				start = 15 + this.shift,
				len = astrology.SYMBOL_SIGNS.length;
			i < len;
			i++
		) {
			var position = astrology.utils.getPointPosition(
				this.cx,
				this.cy,
				this.radius - this.radius / astrology.INNER_CIRCLE_RADIUS_RATIO / 2,
				start
			);
			wrapper.appendChild(
				this.paper.getSymbol(astrology.SYMBOL_SIGNS[i], position.x, position.y)
			);
			start += step;
		}
	};
	...
	astrology.Radix.prototype.drawPoints = function () {
		...
		this.locatedPoints.forEach(function (point) {
			...
			var pointerColor = (astrology.PLANET_COLORS && astrology.PLANET_COLORS[point.name]) || astrology.CIRCLE_COLOR;
			...
			var textsToShow = [
				(Math.round(this.data.planets[point.name][0]) % 30).toString(),
			];
			...
			var pointDescriptions = astrology.utils.getDescriptionPosition(
				point,
				textsToShow
			);
			var pointTextColor = (astrology.PLANET_COLORS && astrology.PLANET_COLORS[point.name]) || astrology.TEXT_COLOR;
	...
	astrology.Radix.prototype.drawAxis = function () {
		...
		[AS, IC, DC, MC].forEach(function (i) {
			...
			if (i == AS) {
				// Text
				let textPosition = astrology.utils.getPointPosition(
					this.cx,
					this.cy,
	...
	astrology.Radix.prototype.aspects = function (customAspects) {
		...
		for (var i = 0, ln = aspectsList.length; i < ln; i++) {
			...
			if (duplicateCheck.indexOf(opositeKey) == -1) {
				...
				line.setAttribute(
					"stroke-width",
					(astrology.ASPECTS_STROKE || astrology.CUSPS_STROKE) * astrology.SYMBOL_SCALE
				);
	...
	astrology.Radix.prototype.drawCircles = function drawCircles() {
		...
		circle.setAttribute(
			"stroke-width",
			astrology.CIRCLE_STRONG * astrology.SYMBOL_SCALE
		);
		wrapper.appendChild(circle);
...
(function (astrology) {
	...
	astrology.Transit = function (radix, data) {
		...
		this.universe.setAttribute(
			"id",
			this.paper.elementId + "-" + astrology.ID_TRANSIT
		);
		this.paper.root.appendChild(this.universe);
	...
	astrology.Transit.prototype.drawPoints = function (planetsData) {
		...
		if (astrology.DEBUG)
			console.log("Transit count of points: " + this.locatedPoints.length);
		if (astrology.DEBUG)
			console.log(
		...
		this.locatedPoints.forEach(function (point) {
			...
			startPosition = astrology.utils.getPointPosition(
				...
				this.cy,
				pointerRadius,
				planets[point.name][0] + this.shift
			);
			endPosition = astrology.utils.getPointPosition(
				this.cx,
				this.cy,
			...
			if (
				!astrology.STROKE_ONLY &&
				planets[point.name][0] + this.shift != point.angle
			) {
				startPosition = endPosition;
				endPosition = astrology.utils.getPointPosition(
					this.cx,
			...
			var zodiac = new astrology.Zodiac(this.data.cusps);
			if (
				planets[point.name][1] &&
				zodiac.isRetrograde(planets[point.name][1])
			) {
				textsToShow.push("R");
			} else {
				textsToShow.push("");
			...
			textsToShow = textsToShow.concat(
				zodiac
					.getDignities(
						{ name: point.name, position: planets[point.name][0] },
						astrology.DIGNITIES_EXACT_EXALTATION_DEFAULT
					)
					.join(",")
			);
	...
	astrology.Transit.prototype.drawCusps = function (cuspsData) {
		...
		for (var i = 0, ln = cusps.length; i < ln; i++) {
			// Lines
			var bottomPosition, startPosition = (bottomPosition = astrology.utils.getPointPosition(
				this.cx,
				this.cy,
				this.radius,
				cusps[i] + this.shift
			));
			var endPosition = astrology.utils.getPointPosition(
				this.cx,
				this.cy,
				this.radius +
					this.radius / astrology.INNER_CIRCLE_RADIUS_RATIO -
	 ...
	 */
	astrology.Transit.prototype.animate = function (
		data,
		duration,
		isReverse,
		callback
					...
					if (this.toPoints.hasOwnProperty(toPoint)) {
						for (var aspect in this.settings.aspects) {
							if (
								...
								// +1 : is moving away
								if (
									isTransitPointApproachingToAspect(
										this.settings.aspects[aspect]["degree"],
										this.toPoints[toPoint][0],
										points[point][0]
									)
	...
	function isTransitPointApproachingToAspect(aspect, toPoint, point) {
		if (point - toPoint > 0) {
			if (point - toPoint > astrology.utils.radiansToDegree(Math.PI)) {
				...
				toPoint =
					(toPoint + aspect) % astrology.utils.radiansToDegree(2 * Math.PI);
			}
		} else {
...
(function (astrology) {
	...
	astrology.Zodiac.prototype.getDignities = function (planet, exactExaltation) {
		...
		var position =
			planet.position % astrology.utils.radiansToDegree(2 * Math.PI);
			...
			for (var i = 0, ln = exactExaltation.length; i < ln; i++) {
				if (planet.name == exactExaltation[i].name) {
					if (
						hasConjunction(
							planet.position,
							exactExaltation[i].position,
							exactExaltation[i].orbit
						)
					) {
	...
	function hasConjunction(planetPosition, pointPosition, orbit) {
		...
		var minOrbit =
			pointPosition - orbit / 2 < 0
				? astrology.utils.radiansToDegree(2 * Math.PI) -
...
(function (astrology) {
	 ...
	 */
	astrology.Animator.prototype.animate = function (
		data,
		duration,
		isReverse,
		callback
	...
	astrology.Animator.prototype.update = function (deltaTime) {
		...
		var expectedNumberOfLoops =
			this.duration - this.timeSinceLoopStart < deltaTime
				? 1
	...
	function updateCusps(expectedNumberOfLoops) {
		...
		context.cuspsElement.setAttribute(
			"transform",
			"rotate(" +
				context.rotation +
				" " +
	...
	function updatePlanets(expectedNumberOfLoops) {
		for (var planet in context.data.planets) {
			...
			var isRetrograde =
				context.actualPlanetPos[planet][1] != null &&
				context.actualPlanetPos[planet][1] < 0;
...
(function (astrology) {
	...
	astrology.utils.getEmptyWrapper = function (parent, elementID) {
		...
		if (wrapper) {
			...
			wrapper = document.createElementNS(
				document.getElementById(astrology._paperElementId).namespaceURI,
				"g"
			);
	...
	astrology.utils.getRulerPositions = function (
		...
		for (
			var i = 0, start = 0, step = astrology.RULER_DEGREES;
			i < 360 / astrology.RULER_DEGREES;
			i++
		) {
			...
			var endPos = astrology.utils.getPointPosition(
				centerX,
				centerY,
				i % astrology.LARGE_RULER_EVERY == 0 ? rayRadius : halfRayRadius,
				angle
			);
			result.push({
				startX: startPos.x,
